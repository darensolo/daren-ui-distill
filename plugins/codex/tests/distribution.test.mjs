...
test('temporary install, launcher, and uninstall preserve user artifacts', async (t) => {
  ...
  assert.equal(result.status, 0, result.stderr);
  const installed = path.join(temp, 'plugins/daren-ui-distill');
  ...
  result = spawnSync(process.execPath, [path.join(installed, 'bin/daren-ui-distill.mjs'), 'check', '--json'], { encoding: 'utf8' });
  assert.equal(result.status, 0, result.stderr);
  ...
  result = spawnSync(process.execPath, [path.join(installed, 'bin/daren-ui-distill.mjs'), 'run', '--base-dir', temp, '--input-kind', 'blueprint', '--reproduction-depth', 'source+adapt', '--target-design-system', 'daren', '--out-dir', 'launcher-adapt', '--json'], { input: JSON.stringify({ blueprint, targetRef }), encoding: 'utf8' });
  assert.equal(result.status, 0, result.stderr);
